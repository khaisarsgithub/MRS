ActiveAdmin.register Quotation do
  ...
  permit_params :phone, :name, :address, :city, :state, :pincode, :quotation_date, :total_area, :total_units, :is_bill, :total_amt, :advance, :balance, :s_gst, :c_gst, :grand_total, quotation_items_attributes: [:id, :name,:width, :height, :quantity,:price_per_unit, :item_id, :quotation_id]
  ...
  index download_links: [:csv] do
    ...
    column 'Download' do |resource|
        ...
        link_to("Bill", without_gst_generate_path(resource, quotation: resource), method: :post),
        link_to("GST Bill", bill_generate_path(resource, quotation: resource), method: :post)
      ], tag(:br))
    end
    actions
  end
